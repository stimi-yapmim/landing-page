...
import { getArticleByIdAction, updateArticleAction, uploadImageAction, getNewsCategoriesAction, createNewsCategoryAction } from "@/app/actions/newsActions";
    ...
    async function loadData() {
      try {
        ...
        if (article) {
          setFormData({
            title: article.title || "",
            category: article.category || "Akademik",
            categoryColor: article.categoryColor || "cyan",
            author: article.author || "Humas STIMI YAPMI",
            ...
            coverGradient: article.coverGradient || "from-brand-navy-950 via-brand-navy-800 to-brand-cyan-600",
            coverAccent: article.coverAccent || "#00bacf",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.success) {
        ...
        setError("Gagal memperbarui berita: " + res.error);
        setLoading(false);
      }
    } catch (err) {
      console.error(err);
      setError("Kesalahan sistem saat memperbarui berita.");
      setLoading(false);
    }
  };
  ...
  if (initialLoading) {
    return (
      <div className="py-20 text-center">
        ...
        <p className="mt-4 text-sm font-semibold text-slate-500 dark:text-slate-400">Mengambil data berita...</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="flex items-center gap-4">
        <Link
          href="/admin/news"
          className="inline-flex h-9 w-9 items-center justify-center rounded-full border border-slate-200 dark:border-slate-800 bg-white dark:bg-brand-navy-900 hover:bg-slate-100 text-slate-750 transition-colors"
        >
          ...
          <div className="bg-white dark:bg-brand-navy-900 rounded-3xl p-6 sm:p-8 shadow-sm border border-slate-200/50 dark:border-slate-800/50 space-y-6">
            ...
            <div>
              ...
              <input
                ...
                value={formData.tags}
                onChange={handleChange}
                placeholder="contoh: Beasiswa, KIP Kuliah, PMB"
                className="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-brand-navy-950 text-sm text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-brand-cyan-500/20 focus:border-brand-cyan-500"
              />
            </div>
          ...
          <div className="bg-white dark:bg-brand-navy-900 rounded-3xl p-6 shadow-sm border border-slate-200/50 dark:border-slate-800/50 space-y-5">
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-3 py-2 rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-brand-navy-950 text-xs text-slate-700 dark:text-slate-250 focus:outline-none focus:ring-2 focus:ring-brand-cyan-500/20 font-mono"
              />
            </div>
